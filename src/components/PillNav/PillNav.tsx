...
const PillNav: React.FC<PillNavProps> = ({
  ...
  const didInitialLoadAnimationRef = useRef(false);
  const itemsKey = items.map(({ href, label }) => `${href}:${label}`).join('|');
  ...
  useEffect(() => {
    ...
    if (initialLoadAnimation && !didInitialLoadAnimationRef.current) {
      didInitialLoadAnimationRef.current = true;
      if (logoRef.current) {
  ...
  }, [itemsKey, ease, initialLoadAnimation]);
  ...
  const isRouter = (href?: string) => !!href && !isExternal(href);
  ...
  return (
    <div className={`pill-nav-container ${className}`}>
      <nav className="pill-nav" style={cssVars} aria-label="Primary">
        {isRouter(items?.[0]?.href) ? (
          <Link
            to="/"
            className="pill-logo"
            aria-label={logoAlt}
            onMouseEnter={handleLogoEnter}
            ref={(el) => { logoRef.current = el as any; }}
          >
        ...
        {trailing && <div className="pill-nav-trailing desktop-only">{trailing}</div>}
      ...
      <div className="mobile-menu-popover mobile-only" ref={mobileMenuRef} style={cssVars}>
        <ul className="mobile-menu-list">
          ...
          {trailing && <li style={{ padding: '8px 12px' }}>{trailing}</li>}
        </ul>
      </div>
    </div>
  );
